...
#define		FSYNC_1()     	GPIO_SetBits(GPIOE,GPIO_Pin_6)//输出高
#define		FSYNC_0()   	GPIO_ResetBits(GPIOE,GPIO_Pin_6)//输出低
#define   SCK_1()			GPIO_SetBits(GPIOE,GPIO_Pin_5)//输出高
#define 	SCK_0()			GPIO_ResetBits(GPIOE,GPIO_Pin_5)//输出低
#define 	DAT_1()			GPIO_SetBits(GPIOE,GPIO_Pin_4)//输出高
#define 	DAT_0()			GPIO_ResetBits(GPIOE,GPIO_Pin_4)//输出低

#define		FSYNC2_1()     	GPIO_SetBits(GPIOE,GPIO_Pin_3)//输出高
#define		FSYNC2_0()   	GPIO_ResetBits(GPIOE,GPIO_Pin_3)//输出低
#define   SCK2_1()			GPIO_SetBits(GPIOE,GPIO_Pin_2)//输出高
#define 	SCK2_0()			GPIO_ResetBits(GPIOE,GPIO_Pin_2)//输出低
#define 	DAT2_1()			GPIO_SetBits(GPIOE,GPIO_Pin_1)//输出高
#define 	DAT2_0()			GPIO_ResetBits(GPIOE,GPIO_Pin_1)//输出低
...
void AD9833_WaveSeting(double Freq,unsigned int Freq_SFR,unsigned int WaveMode,unsigned int Phase,uint8_t drive_id )
{
		...
		if(Freq_SFR==0)				  //把数据设置到设置频率寄存器0
		{
			...
			//AD9833_Write(0x2000,drive_id); /**设置FSELECT位为0，芯片进入工作状态,频率寄存器0输出波形**/
	    }
		if(Freq_SFR==1)				//把数据设置到设置频率寄存器1
		{
			...
			AD9833_Write(Phs_data,drive_id);	//设置相位
			//AD9833_Write(0x2800,drive_id); /**设置FSELECT位为0，设置FSELECT位为1，即使用频率寄存器1的值，芯片进入工作状态,频率寄存器1输出波形**/
		}
