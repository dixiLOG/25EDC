...
/* 【LMS滤波器】的两个输入一定要软件滤除直流分量，否则其强相关性会使得结果趋向0；注意去噪模式下，pSrc为参考信号，Ref为混叠信号 */
...
//   sum：求和结果
//   blockSize： 数组大小
//  调用： DSP_SUM(pSrcA,sum,blockSize);
//======
...
void DSP_LMS_FILTER( float32_t *pSrc , float32_t *pRef , float32_t *pDst , float32_t *err , uint16_t inputLen , float32_t *pCoeffs , uint16_t numTaps , float32_t mu, float32_t *state,arm_lms_instance_f32 *S);
// pSrc: 输入信号（干扰信号/参考信号）
// pRef: 期望信号（目标信号）
// pDst: 滤波后输出信号
...
// 参考：https://www.cnblogs.com/LFYMay23/p/12582132.html
...
// state:缓存buffer
// 注意：输入需要先去除直流分量（已集成）
// 调用：DSP_LMS_NORM_FILTER(refInput, testInput, lmsOutput, errorOutput, TEST_LENGTH_SAMPLES,fir_coeffs, NUM_TAPS, mu,lms_state);
//======
