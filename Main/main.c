...
// DMA ping-pong 状态参数
volatile int8_t ADC_Data_Ready_Buffer = -1; // -1:无数据, 0:前半段准备好, 1:后半段准备好
...
// > 具体参见 `25EDCForAdaF` 项目(https://github.com/dixiLOG/25EDC/tree/25EDCforAdaF)
 ...
 * @note   此函数由DMA完成标志触发。它调用FFT处理函数，然后在LCD屏幕上绘制生成的单边幅度频谱。
 * @note   为避免DMA的pipline和CPU的FFT处理冲突，采用ping-pong处理。
 */
void FFT_SHOW() {
    // 检查ADC采样和DMA传输是否完成
    if (DMA_FLAG) {
        ...
        // =======ping-pong 处理=======
        if (ADC_Data_Ready_Buffer == 0) {
            // 前半段数据好了，处理前半段，DMA正在填充后半段
            DSP_FFT(&ADC1_Data_Rx[0]); 
        } else if (ADC_Data_Ready_Buffer == 1) {
            // 后半段数据好了，处理后半段，DMA正在填充前半段
            DSP_FFT(&ADC1_Data_Rx[FFT_N]);
        }
        
        ADC_Data_Ready_Buffer = -1; // 将标志复位
				// ============================
        ...
        // 绘制单边频谱图
        for (u16 x = 0; x <= 230; x++) {
					float32_t Mag_Draw = (Mag_Single[x] >= 100)?100:Mag_Single[x];
					if(Mag_max < 0.1) Mag_Draw = 0.01;															// 限幅
            LCD_Color_DrawLine(5 + x, 315 - Mag_Draw, 5 + x, 315, BLUE);
        }
    }
}
...
//						delay_ms(500); // 一秒两刷
				...
				sprintf(String, "Mag_max_index = %.3f Hz",Mag_max_index);
